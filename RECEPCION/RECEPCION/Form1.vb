...
Public Class Form1
    ...
    Private Sub CargarCombobox_rutas()
        If bandera = 1 Then
            Try
                ...
                ' Dim sqlConsult As String = "select distinct catrut.id_ruta as id_RUTA,catrut.nombre_ruta as NOMBRE_RUTA from CATALOGO_RUTA catrut " & _
                ' " join centro_votacion cv on catrut.id_ruta=cv.id_ruta " & _
                ' " join municipio mun on cv.id_municipio= mun.id_municipio " & _
                ' " join sede_logistica sed on mun.id_sede= sed.id_sede " & _
                ' " join departamento dep on dep.id_departamento= sed.id_departamento " & _
                '" where cv.id_sede = sed.id_sede And cv.id_sede = mun.id_sede And cv.id_departamento = mun.id_departamento " & _
                '" and cv.id_departamento=sed.id_departamento and cv.id_departamento= dep.id_departamento and " & _
                '" cv.id_municipio = mun.id_municipio And mun.id_sede = sed.id_sede And mun.id_departamento = dep.id_departamento " & _
                '" and sed.id_departamento=dep.id_departamento " & _
                ' " and sed.id_sede=:SEDE order by 1 "
                Dim sqlConsult As String = "Select * from catalogo_ruta"
                Dim comando As New OracleCommand(sqlConsult, conn)
                ' MessageBox.Show(ComboBox1.SelectedValue.ToString)
                'comando.Parameters.Add(":SEDE", OracleType.Int32).Value = ComboBox1.SelectedValue.ToString
                Dim dataAdapter As New OracleDataAdapter(sqlConsult, conn)
                Dim DT As New DataTable
                dataAdapter.Fill(DT)
        ...
        Try
            ...
            If lector.HasRows Then
                ...
                For Each dr As DataGridViewRow In DataGridView1.Rows
                    dt.Rows.Add(dr.Cells("NOMBRE_DEPARTAMENTO").Value, dr.Cells("NOMBRE_SEDE").Value, _
                    dr.Cells("NOMBRE_MUNICIPIO").Value, dr.Cells("NOMBRE_CENTRO_VOTACION").Value, _
    ...
    '    Try
    '        Dim sqlConsult As String = "select * from reporte_recepcion_view where sede=:sede and trim(ruta)=:ruta and id_catalogo_recibida=2"
    '        'Dim sqlConsult As String = "select * from reporte_recepcion_view where trim(RUTA) = '" & ComboBox2.SelectedValue.ToString.Trim & _
    '        '    "' AND SEDE = " & ComboBox1.SelectedValue.ToString & " and id_catalogo_recibida=2"
    '        conn.Open()
    ...
    'Private Sub ComboBox1_SelectedvalueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ComboBox1.SelectedIndexChanged
    '    CargarCombobox_rutas()
    'End Sub
